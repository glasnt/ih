...
def chart(image_name=None, image_obj=None, palette_name="wool", scale=1, colours=256, render=False, fileformat="html", save=True):
    if image_name: 
...
def generate_chart(chartimage, palette_name, render=False):
    ...
    html.append(f'<div class="debug">Image: {chartimage.width} x {chartimage.height}. ih version {ih_version}</div>')
